(() => {
  ...
  function viewContract(){
    ...
    content.innerHTML=`${pageHead('DOCUMENTOS COMERCIAIS','Meu <em>contrato.</em>','Aqui ficam separados o contrato enviado para assinatura e a via assinada da sua marca.',statusLabel(c.status||'pending'),'STATUS')}
      <div class="two-col">
        <div class="card"><div class="card-row"><div><span class="label">CONTRATO PARA ASSINATURA</span><h3>${generated?'Contrato disponível':'Aguardando geração'}</h3><p>${generated?`Arquivo: ${esc(generated.original_name)}${c.generated_at?` · gerado em ${fmtDateTime(c.generated_at)}`:''}`:'A equipe Comercial ainda não gerou o contrato desta marca.'}</p></div>${status(generated?'pending':'draft')}</div>${generated?`<div class="inline-actions" style="margin-top:18px"><button class="btn btn-dark" id="openGeneratedContract">Abrir contrato ↗</button></div>`:''}</div>
